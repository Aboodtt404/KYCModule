...
export default function ReviewStep({ userData, editedData, onNext }) {
    ...
    return (<GlassCard className="space-y-4">
      ...
      <div className="text-sm text-gray-300">
        ...
        {Object.keys(displayData).length > 0 && (<div className="mt-4 pt-4 border-t border-gray-600">
            <h4 className="font-medium mb-2">Verified Information:</h4>

            {/* Face Image */}
            {displayData.face_image && (<div className="mb-4 p-3 bg-gray-800 rounded-lg">
                <h5 className="text-sm font-medium mb-2">Extracted Face:</h5>
                <img src={`data:image/jpeg;base64,${displayData.face_image}`} alt="Extracted face from ID" className="w-24 h-24 object-cover rounded border border-gray-600"/>
              </div>)}

            {/* Other Fields */}
            {Object.entries(displayData)
                .filter(([key]) => key !== "face_image")
                .map(([key, value]) => (<div key={key} className="flex justify-between mt-1">
