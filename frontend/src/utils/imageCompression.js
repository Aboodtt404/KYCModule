...
export const compressImage = async (file, options = {}) => {
    const { maxSizeKB = 500, // Default 500KB limit
    maxWidth = 1200, maxHeight = 1200, quality = 0.8, format = 'image/jpeg' } = options;
            ...
            try {
                ...
                canvas.toBlob((blob) => {
                    ...
                    if (blob.size > targetSizeBytes) {
                        compressWithQualityReduction(canvas, ctx, img, width, height, targetSizeBytes, format, resolve, reject);
                    }
                    else {
                        resolve({
                            compressedBlob: blob,
 ...
 */
const compressWithQualityReduction = (canvas, ctx, img, width, height, targetSizeBytes, format, resolve, reject, quality = 0.7, minQuality = 0.1) => {
    canvas.toBlob((blob) => {
        if (!blob) {
            reject(new Error('Failed to compress image'));
            return;
        ...
        compressWithQualityReduction(canvas, ctx, img, width, height, targetSizeBytes, format, resolve, reject, newQuality, minQuality);
    }, format, quality);
};
/**
 ...
 */
export const needsCompression = (file, maxSizeKB = 1000) => {
 ...
 */
export const createCompressedFile = (originalFile, compressedBlob) => {
