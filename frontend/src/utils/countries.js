/**
 * Country data sourced from Ballerine open-source KYC platform
 * (packages/common/src/countries/index.ts — ISO 3166-1)
 * Extended with alpha-3 codes for MRZ passport nationality resolution.
 */
...
const countries = [
  ...
  { name: 'Congo, Democratic Republic',                       'alpha-2': 'CD', 'alpha-3': 'COD' },
  ...
  { name: 'Palestine',                                        'alpha-2': 'PS', 'alpha-3': 'PSE' },
  ...
  { name: 'Taiwan',                                           'alpha-2': 'TW', 'alpha-3': 'TWN' },
...
// Build lookup maps once at module load — O(1) lookups at runtime
const byAlpha2 = Object.fromEntries(countries.map(c => [c['alpha-2'], c.name]));
const byAlpha3 = Object.fromEntries(countries.map(c => [c['alpha-3'], c.name]));

/**
 * Resolve a country code (alpha-2 or alpha-3 / MRZ) to a human-readable name.
 * Returns the original code unchanged if not found, so display never goes blank.
 *
 * Examples:
 *   countryCodeToName('EGY') → 'Egypt'
 *   countryCodeToName('EG')  → 'Egypt'
 *   countryCodeToName('GBR') → 'United Kingdom'
 *   countryCodeToName('USA') → 'United States'
 */
export function countryCodeToName(code) {
  if (!code) return '';
  const upper = String(code).toUpperCase().trim();
  return byAlpha2[upper] ?? byAlpha3[upper] ?? code;
}

/** All countries as { value, label } pairs — useful for select dropdowns. */
