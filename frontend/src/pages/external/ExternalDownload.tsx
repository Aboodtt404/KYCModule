...
export function ExternalDownload() {
  ...
  const getFileIcon = (type: string) => {
    ...
    if (type.includes("Text")) return <FileText className="w-6 h-6 text-green-500" />;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {mockFiles.map((file) => {
          ...
          return (
            <div
              key={file.id}
              className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-xl shadow-sm p-5 flex flex-col justify-between"
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                {status ? (
                  <span
                    className={`flex items-center space-x-1 text-xs font-medium px-2 py-1 rounded-full ${status.color}`}
                  >
                    {status.icon}
