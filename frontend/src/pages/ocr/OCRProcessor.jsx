...
export function OCRProcessor() {
    ...
    return (<div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6 border-b border-gray-200 dark:border-gray-700 flex space-x-6">
        <button onClick={() => setActiveTab("process")} className={`px-4 py-2 -mb-px font-medium border-b-2 transition ${activeTab === "process"
            ? "border-blue-600 text-blue-600 dark:text-blue-400"
            : "border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"}`}>
          OCR Processor
        </button>
        <button onClick={() => setActiveTab("results")} className={`px-4 py-2 -mb-px font-medium border-b-2 transition ${activeTab === "results"
            ? "border-blue-600 text-blue-600 dark:text-blue-400"
            : "border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"}`}>
          Stored Results
        </button>
        <button onClick={() => setActiveTab("debug")} className={`px-4 py-2 -mb-px font-medium border-b-2 transition ${activeTab === "debug"
            ? "border-blue-600 text-blue-600 dark:text-blue-400"
            : "border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"}`} disabled={!debugInfo}>
      ...
      {activeTab === "process" && (<div>
          ...
          <div className="flex space-x-6 mb-6">
            <button onClick={() => setOcrType("egyptian")} className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition ${ocrType === "egyptian"
                ? "bg-blue-100 text-blue-700 dark:bg-blue-600/20 dark:text-blue-300"
                : "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"}`}>
              <Flag className="w-4 h-4 mr-2"/>
              Egyptian ID
            </button>
            <button onClick={() => setOcrType("passport")} className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition ${ocrType === "passport"
                ? "bg-blue-100 text-blue-700 dark:bg-blue-600/20 dark:text-blue-300"
                : "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm p-6">
              ...
              {imageFiles.length === 0 ? (<div className="text-center py-8 text-gray-600 dark:text-gray-400">
                  ...
                  {imageFiles.map((file) => (<button key={file.path} onClick={() => handleImageSelect(file)} className={`w-full text-left px-3 py-2 rounded-lg border transition ${selectedImage?.path === file.path
            ...
            <div className="lg:col-span-2 bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm p-6">
              ...
              {!selectedImage ? (<div className="text-center py-12 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-500 dark:text-gray-400">
                  ...
                  {/* Compression Status */}
                  {compressing && (<div className="flex items-center justify-center space-x-2 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                      <Zap className="w-5 h-5 text-blue-500 animate-pulse"/>
                      <span className="text-blue-700 dark:text-blue-300">Compressing image...</span>
                    </div>)}

                  {compressionInfo && (<div className="p-3 bg-green-50 dark:bg-green-900/30 rounded-lg">
                  ...
                  <div className="text-center">
                    <button onClick={handleRunOcr} disabled={ocrMutation.isPending ||
                    egyptianIDMutation.isPending ||
                    passportMutation.isPending ||
                    compressing} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition flex items-center justify-center mx-auto disabled:opacity-50">
                    ...
                    !passportMutation.isPending &&
                    !compressing && (<>
                            {ocrType === "egyptian" ? (<Flag className="w-5 h-5 mr-2"/>) : (<ScanText className="w-5 h-5 mr-2"/>)}
                            Run {ocrType === "egyptian" ? "Egyptian ID" : "Passport"} OCR
                          </>)}
                    </button>
                  </div>

                  {error && (<div className="mt-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg text-sm text-red-800 dark:text-red-300">
                      <AlertCircle className="w-4 h-4 inline mr-2"/>
                      {error}
                    </div>)}

                  {/* Egyptian ID Results */}
                  {egyptianIDResult && (<div className="mt-6 space-y-4">
                  ...
                  {/* Passport Results */}
                  {passportResult && (<div className="mt-6 space-y-6">
      ...
      {/* Results Tab */}
      {activeTab === "results" && (<div className="mt-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-xl p-6">
              ...
              {!egyptianIdResults || egyptianIdResults.length === 0 ? (<p className="text-gray-500 dark:text-gray-400 text-center py-6">
                  No results stored
                </p>) : (<div className="space-y-3 max-h-96 overflow-y-auto">
                  {egyptianIdResults.map(([path, data], idx) => {
                    try {
                        const parsed = JSON.parse(data);
                        return (<div key={idx} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        return (<p key={idx} className="text-sm text-red-600 dark:text-red-400">
                          Error parsing {path}
                        </p>);
                    }
                })}
            ...
            <div className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-xl p-6">
              ...
              {!passportResults || passportResults.length === 0 ? (<p className="text-gray-500 dark:text-gray-400 text-center py-6">
                  No results stored
                </p>) : (<div className="space-y-3 max-h-96 overflow-y-auto">
      ...
      {/* Debug Tab */}
      {activeTab === "debug" && debugInfo && (<div className="mt-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 flex items-center mb-4">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-xl p-6">
              ...
              {debugInfo.detected_fields && debugInfo.detected_fields.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {debugInfo.detected_fields.map((field, index) => (<div key={index} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                ...
                </div>) : (<p className="text-gray-500 dark:text-gray-400 text-center py-6">
            ...
            <div className="bg-white dark:bg-glass border border-gray-200 dark:border-white/10 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {debugInfo.preprocessed_image_path && (<a href={`http://194.31.150.154:5000/debug-image/${debugInfo.preprocessed_image_path}`} target="_blank" rel="noopener noreferrer" className="block p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                ...
                {debugInfo.debug_image_path && (<a href={`http://194.31.150.154:5000/debug-image/${debugInfo.debug_image_path}`} target="_blank" rel="noopener noreferrer" className="block p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <div className="flex items-center space-x-2 mb-2">
                ...
                {debugInfo.cropped_image_path && (<a href={`http://194.31.150.154:5000/debug-image/${debugInfo.cropped_image_path}`} target="_blank" rel="noopener noreferrer" className="block p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <div className="flex items-center space-x-2 mb-2">
                ...
                {debugInfo.yolo_output_path && (<a href={`http://194.31.150.154:5000/debug-image/${debugInfo.yolo_output_path}`} target="_blank" rel="noopener noreferrer" className="block p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <div className="flex items-center space-x-2 mb-2">
                ...
                {debugInfo.mrz_roi_path && (<a href={`http://194.31.150.154:5000/debug-image/${debugInfo.mrz_roi_path}`} target="_blank" rel="noopener noreferrer" className="block p-4 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <div className="flex items-center space-x-2 mb-2">
            ...
            {/* Technical Details */}
            {debugInfo.mrz_detected && (<div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center">
