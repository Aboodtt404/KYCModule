...
export default function DocumentUpload() {
    ...
    return (<div className="space-y-6">
      <div>
        ...
        {/* File Upload Area */}
        <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
          {uploadState.file ? (<div className="space-y-4">
              ...
              <div>
                ...
                {/* Compression Status */}
                {uploadState.compressing && (<div className="flex items-center space-x-2 mt-2">
                    <Zap className="w-4 h-4 text-blue-500 animate-pulse"/>
                    <span className="text-sm text-blue-600">Compressing image...</span>
                  </div>)}

                {uploadState.compressionInfo && (<div className="mt-2 p-2 bg-green-50 rounded-md">
              ...
              <button onClick={resetUpload} className="text-sm text-blue-600 hover:text-blue-700">
              ...
              <label htmlFor="file-upload" className="inline-block bg-blue-600 text-white px-6 py-2 rounded-lg cursor-pointer hover:bg-blue-700 transition-colors">
                Browse Files
              </label>
            </div>)}
        </div>
        ...
        {/* Image Preview */}
        {(uploadState.originalImageUrl || uploadState.enhancedImageUrl) && (<div className="mt-6 bg-white border rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-4">Image Preview</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {uploadState.originalImageUrl && (<div>
                  <p className="text-sm font-medium text-gray-700 mb-2">Original Image</p>
                  <img src={uploadState.originalImageUrl} alt="Original document" className="w-full h-48 object-contain border rounded-lg bg-gray-50"/>
                </div>)}
              {uploadState.enhancedImageUrl && (<div>
                  <p className="text-sm font-medium text-gray-700 mb-2">Enhanced Image</p>
                  <img src={uploadState.enhancedImageUrl} alt="Enhanced document" className="w-full h-48 object-contain border rounded-lg bg-gray-50"/>
                </div>)}
            </div>
          </div>)}
        ...
        {/* Progress Indicators */}
        {(uploadState.enhancing || uploadState.uploading || uploadState.processing) && (<div className="mt-6 space-y-4">
            ...
            {/* Enhancement Details */}
            {uploadState.enhancing && (<div className="bg-gray-50 rounded-lg p-4">
        ...
        {/* Error State */}
        {uploadState.error && (<div className="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-center space-x-3">
            ...
            <button onClick={resetUpload} className="mt-3 text-sm text-red-600 hover:text-red-700 font-medium">
              Try Again
            </button>
          </div>)}

        {/* Success State */}
        {uploadState.completed && uploadState.rawOcrResult && (<div className="mt-6 space-y-6">
            ...
            {uploadState.structuredData && (<StructuredDataDisplay data={uploadState.structuredData}/>)}
