    ...
    return (<motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="w-full max-w-md mx-auto text-center space-y-4 sm:space-y-6 bg-gray-900/40 backdrop-blur-lg p-4 sm:p-8 rounded-xl sm:rounded-2xl shadow-lg border border-gray-800">
        ...
        <AnimatePresence mode="wait">
            {step === "phone" ? (<motion.div key="phone" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-3 sm:space-y-4">
                ...
                <Button className="w-full h-10 sm:h-12 text-sm sm:text-lg font-semibold rounded-lg sm:rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white shadow-md" onClick={handleSendCode} disabled={loading || !phone}>
                    {loading ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> : "Send Code"}
                </Button>

                <Button
                    variant="outline"
                    className="w-full h-10 sm:h-12 text-xs sm:text-sm font-medium rounded-lg sm:rounded-xl border-gray-700 hover:bg-gray-800 text-gray-300"
                ...
                <div className="flex justify-center space-x-2 sm:space-x-3">
                    ...
                    }} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleChange(e.target.value, i)} onKeyDown={(e) => handleKeyDown(e, i)} className="w-10 h-12 sm:w-12 sm:h-14 text-center text-lg sm:text-xl font-bold bg-gray-800 border border-gray-700 text-white focus:ring-emerald-500 focus:border-emerald-500 rounded-lg sm:rounded-xl" />))}
                </div>

                <Button className="w-full h-10 sm:h-12 text-sm sm:text-lg font-semibold rounded-lg sm:rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white shadow-md" onClick={handleVerifyCode} disabled={loading || otp.join("").length !== 6}>
                ...
                <div className="flex justify-between items-center text-xs sm:text-sm text-gray-400 gap-2">
                    ...
                    <button onClick={handleSendCode} disabled={timer > 0} className={`${timer > 0 ? "text-gray-500" : "text-emerald-400 hover:text-emerald-300"} transition truncate`}>
