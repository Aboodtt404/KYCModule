...
export function FieldEditStep({ ocrData, faceImage, onNext, onBack }) {
    ...
    return (<motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -40 }} className="w-full max-w-8xl mx-auto px-4">
      <GlassCard className="p-8 sm:p-12">
        <div className="text-center mb-12">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: "spring", stiffness: 200 }} className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center">
            <Edit3 className="w-8 h-8 text-white"/>
          </motion.div>

          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            Verify Identity & Edit Information
          </motion.h2>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-gray-600 dark:text-gray-300">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <div className="w-full h-64 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center overflow-hidden">
                  {faceImage ? (<img src={`data:image/jpeg;base64,${faceImage}`} alt="Face from ID" className="w-full h-full object-cover"/>) : (<div className="text-gray-500 dark:text-gray-400">No face detected</div>)}
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="space-y-3">
                  <div className="w-full h-64 bg-gray-100 dark:bg-gray-800 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors border-2 border-dashed border-gray-300 dark:border-gray-600" onClick={() => setShowCamera(true)}>
                    {selfiePreview ? (<img src={selfiePreview} alt="Selfie preview" className="w-full h-full object-cover rounded-lg"/>) : (<div className="text-center">
                  ...
                  {selfieFile && (<div className="flex items-center justify-center space-x-2">
                      <div className="flex flex-col items-center">
                        <p className="text-xs text-gray-600 dark:text-gray-400">
                          Selfie captured: {selfieFile.name}
                        </p>
                        {lastSelfieTimestamp && (<p className="text-xs text-blue-600 dark:text-blue-400">
                            Captured: {new Date(lastSelfieTimestamp).toLocaleTimeString()}
                          </p>)}
                      </div>
                      <Button size="sm" variant="outline" onClick={() => setShowCamera(true)} className="text-xs h-6 px-2">
                        Retake
                      </Button>
                    </div>)}
                </div>
              ...
              {/* Verification Status */}
              {verificationResult && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`p-4 rounded-lg border ${verificationResult.is_match
                ...
                : "bg-red-100 dark:bg-red-800"}`}>
                      {verificationResult.is_match ? (<CheckCircle className="w-5 h-5 text-emerald-600 dark:text-emerald-400"/>) : (<X className="w-5 h-5 text-red-600 dark:text-red-400"/>)}
                    </div>
                ...
                : "text-red-600 dark:text-red-400"}`}>
                        ...
                        {lastSelfieTimestamp && (<span className="ml-2 text-gray-500">
              ...
              {/* Verify Button */}
              {!verificationResult && (<div className="space-y-2">
                  <Button onClick={handleVerifyIdentity} disabled={isVerifying || !selfieFile} className="w-full h-12 text-lg font-semibold bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isVerifying ? (<div className="flex items-center space-x-2">
                  ...
                  {lastSelfieTimestamp && (<p className="text-xs text-center text-blue-600 dark:text-blue-400">
                      ✨ Fresh selfie captured - verification will use new image
                    </p>)}
                </div>)}

              {/* Retry Verification Button */}
              {verificationResult && !verificationResult.is_match && (<Button onClick={handleVerifyIdentity} disabled={isVerifying || !selfieFile} className="w-full h-12 text-lg font-semibold bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                  {isVerifying ? (<div className="flex items-center space-x-2">
                      <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              ...
              {verificationResult && verificationResult.is_match && (<Button onClick={() => {
                setVerificationResult(null);
                setIsIdentityVerified(false);
                setSelfieFile(null);
                setSelfiePreview(null);
                setLastSelfieTimestamp(null);
            }} variant="outline" className="w-full h-10 text-sm font-medium border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <div className="space-y-8">
            <div>
              ...
              {!isIdentityVerified ? (<div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 mb-6">
                  <div className="flex items-center space-x-3">
                ...
                </div>) : (<p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                  ✅ Identity verified! You can now edit the extracted information.
                </p>)}
            </div>

            <div className="space-y-6">
              {fieldsToShow.map(({ key, label, required }) => (<div key={key} className="space-y-2">
                  <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {label} {required && <span className="text-red-500">*</span>}
                  </Label>

                  {editingField === key ? (<div className="flex space-x-2">
                      <Input value={getFieldValue(key, fieldsToShow.find(f => f.key === key)?.fallbackKeys)} onChange={(e) => handleFieldEdit(key, e.target.value)} className="flex-1" autoFocus disabled={!isIdentityVerified}/>
                      <Button size="sm" onClick={() => handleSaveField(key)} className="bg-emerald-500 hover:bg-emerald-600" disabled={!isIdentityVerified}>
                        <Save className="w-4 h-4"/>
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => setEditingField(null)}>
                        <X className="w-4 h-4"/>
                      </Button>
                    </div>) : (<div className={`flex items-center justify-between p-4 rounded-lg border transition-colors ${isIdentityVerified
                    ? "bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700"
                    : "bg-gray-100 dark:bg-gray-900 border-gray-300 dark:border-gray-600 cursor-not-allowed opacity-60"}`} onClick={() => isIdentityVerified && setEditingField(key)}>
      ...
      <CameraCapture isOpen={showCamera} onCapture={handleCameraCapture} onCancel={handleCameraCancel}/>
    </motion.div>);
}
