...
export default function DocumentStep({ onNext, onUploaded, }) {
    ...
    return (<div className="space-y-6">
      ...
      <GlassCard className="p-6">
        ...
        ].map((doc) => (<button key={doc.key} className={`p-3 rounded-xl w-full sm:w-auto transition ${type === doc.key
                ? "bg-emerald-500 text-black font-semibold"
                : "bg-white/10 hover:bg-white/20"}`} onClick={() => setType(doc.key)}>
              {doc.label}
            </button>))}
        </div>
        ...
        {/* Upload */}
        <div className="mt-6">
          <UploadBox label={`Upload ${type === "id" ? "National ID (front)" : "Passport"}`} onFile={handleFile}/>
        </div>

        {/* File details + process button */}
        {file && (<div className="mt-6 text-sm text-gray-200 space-y-3">
            ...
            <Button onClick={handleProcessDocument} className="w-full" disabled={isProcessing}>
